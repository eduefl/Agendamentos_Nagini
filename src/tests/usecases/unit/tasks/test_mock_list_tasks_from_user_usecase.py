...
from usecases.task.list_tasks_from_user.list_tasks_from_user_dto import ListTasksFromUserInputDTO, ListTasksFromUserOutputDTO
...
class TestMockListTasksFromUserUseCase:
	def test_mock_list_tasks_from_user_usecase(self,make_task, make_user):
		...
		mock_task_repository.list_tasks_from_user.return_value = [make_task(user_id=user_id, 
																	  title="Task 1", 
																	  description="Description for Task 1"),
											   						make_task(user_id=user_id, 
							  											title="Task 2", 
																		description="Description for Task 2"
																	)]

		# caso de uso
		use_case = ListTasksFromUserUseCase( task_repository= mock_task_repository, user_repository= mock_user_repository)
