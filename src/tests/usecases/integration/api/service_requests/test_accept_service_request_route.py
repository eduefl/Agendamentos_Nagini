...
class TestAcceptServiceRequestRoute:
    ...
    def _create_available_service_request(
        self, session, client_id, service_id, provider_id=None, provider_price=None
    ):
        service_model = (
    ...
    def test_accept_requires_auth(self, client):
        response = client.patch(
            f"/provider-service-requests/{uuid4()}/accept",
    ...
    def test_accept_returns_404_when_request_not_found(
    ...
    def test_accept_returns_422_when_provider_does_not_serve_service(
        self, client, tst_db_session, make_user, seed_roles
    ):
        session = tst_db_session
        client_user = self._create_user(session, make_user, {"cliente"})
    ...
    def test_accept_returns_409_when_already_accepted_by_another(
        self, client, tst_db_session, make_user, seed_roles
    ):
        session = tst_db_session
        client_user = self._create_user(session, make_user, {"cliente"})
    ...
    def test_accept_returns_409_when_request_unavailable(
        self, client, tst_db_session, make_user, seed_roles
    ):
        session = tst_db_session
        client_user = self._create_user(session, make_user, {"cliente"})
    ...
    def test_accept_response_contains_correct_prices(
        self, client, tst_db_session, make_user, seed_roles
    ):
        session = tst_db_session
        client_user = self._create_user(session, make_user, {"cliente"})
