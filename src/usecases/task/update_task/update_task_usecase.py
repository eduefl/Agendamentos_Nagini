...
class UpdateTaskUseCase(TaskCaseInterface):
	...
	def execute(self, input: UpdateTaskInputDTO) -> UpdateTaskOutputDTO:
		task = self.task_repository.get_task_by_id( task_id=input.id) #Confirmo que a tarefa existe, se não existir, o método get_by_id deve lançar uma exceção
